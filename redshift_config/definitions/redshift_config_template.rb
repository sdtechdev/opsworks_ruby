...
define :redshift_config_template do
  template "#{params[:deploy_to]}/shared/config/redshift.database.yml" do
    cookbook 'redshift_config'
    source "redshift.database.yml.erb"
    owner params[:deploy][:user]
    group params[:deploy][:group]
    mode "0660"
    variables :env => params[:env], :environment => params[:deploy][:global][:environment]

    only_if do
      File.exists?("#{params[:deploy_to]}/shared/config")
    end
  end
